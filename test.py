...
HAM_CAT=0
VOCAB_SIZE=2500
SPARSE_WORD_PATH='./dataset/sparse_train_df.to_csv'
...
#wordの種類ごとに分ける.出現回数をunique_wordに格納.この時点でwordがIndexで出現回数がcolumnデータになっている．
...
# #Create a sparse matrix for the training data.word_indexはvocabのindexを並べたもの
...
# sparse_train_df=pd.read_csv(SPARSE_WORD_PATH)
# sparse_train_df=make_sparse_matrix(x_train,word_index,y_train)
# sparse_train_df.to_csv(SPARSE_WORD_PATH)
...
trained_grouped=pd.read_csv(TRAINING_DATA_FILE)
train_doc_info=trained_grouped.DOC_ID.value_counts()
# print(len(train_doc_info.index))
test_grouped=pd.read_csv(TEST_DATA_FILE)
test_doc_info=test_grouped.DOC_ID.value_counts()
print(len(test_doc_info.index))
